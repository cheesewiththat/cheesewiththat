...
Google Analytics 4 uses the official Next.js integration and loads once from the root layout only for the Amplify `main` production deployment on `cheesewiththat.com` or `www.cheesewiththat.com`. The build requires Amplify’s built-in app and branch context, rejects pull-request context, and deliberately strips analytics configuration from local builds, tests, previews and non-production branches; a client-side hostname allow-list provides a second guard. Client-side route changes rely on GA Enhanced Measurement; enable browser-history page views in the GA property. Interaction events contain only allow-listed public labels, categories and paths—never form values or contact details. Review analytics consent and privacy treatment before broader campaigns.
