...
Google Analytics 4 uses the official `@next/third-parties` component exactly once in the root layout. The Amplify environment writer exposes the measurement ID and a generated enable flag only when built-in Amplify context identifies the `main` branch, an app ID is present, and no pull-request ID exists. A client-side allow-list then requires the exact production apex or `www` hostname before rendering GA or emitting any event. Local builds, tests, previews and non-production branches therefore render no GA component and send no analytics events even if another script defines `window.gtag`. App Router page views rely on GA Enhanced Measurement rather than manual duplicate events. `src/lib/analytics.ts` is a typed, client-only, no-throw event boundary with an explicit public-parameter allow-list and short duplicate suppression; no form values or contact details cross it.

Future integrations: durable rate limiting/deduplication, payment, print fulfilment, newsletter, CMS, secure CV file delivery, accounts, conversational assistant, guided visitor modes, dynamic CV generation, and consent-aware analytics controls.

Each engagement schema explicitly declares a `booking` or `enquiry` workflow. `BookingFlow` renders from those typed schemas and retains state across edit/review steps without inferring behavior from duration or route names. `CalendlyEmbed` loads the supported widget script only for the four booking workflows, validates the Calendly origin and iframe message source, and confirms only `calendly.event_scheduled`. `MihirMap` loads Google Maps only on `/map`, uses one loader promise and advanced markers when configured, omits invalid or hidden marker records before fitting bounds, and otherwise retains the filters and accessible location experience. Places are typed as `lived` or `travelled`; lived markers are more prominent, the current home is identified only at city level, and repeated cities keep ordered life chapters on one marker. Remote photography is restricted to the single HTTPS origin and path prefix configured at build time.
