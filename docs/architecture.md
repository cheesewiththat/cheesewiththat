...
Google Analytics 4 uses the official `@next/third-parties` component exactly once in the root layout. Rendering and event emission require both a configured `NEXT_PUBLIC_GA_MEASUREMENT_ID` and production mode. Amplify pull-request previews are detected with the built-in `AWS_PULL_REQUEST_ID`; the environment writer omits the ID and the preview build command unsets any inherited value before Next.js can embed it. Local development and tests keep the ID blank. No deployment-environment variable, generated enable flag or client hostname gate is used. App Router page views rely on GA Enhanced Measurement rather than manual duplicate events. `src/lib/analytics.ts` is a typed, client-only, no-throw event boundary with an explicit public-parameter allow-list and short duplicate suppression; no form values or contact details cross it.
